...
/** 1-2 character initial derived from the universe name (used in the card box). */
function initials(universe: Universe): string {
  return universe.name.trim().slice(0, 2).toUpperCase()
}
...
function openEdit(universe: Universe) {
  editingId.value = universe.id
  name.value = universe.name
  description.value = universe.description
  modalOpen.value = true
}
...
function save() {
  ...
  const universe = new Universe(name.value.trim())
  universe.description = description.value.trim()

  if (editingId.value) {
    universe.id = editingId.value
    projectValue.universeService.edit(editingId.value, universe)
  } else {
    projectValue.universeService.add(universe)
  }
...
<template>
  ...
  >
    <div class="mx-auto w-full max-w-4xl">
      ...
      <div class="mt-8 flex flex-col gap-4">
        <div
          ...
          role="button"
          tabindex="0"
          @click="$emit('navigate', 'universe-detail', { id: universe.id })"
          @keydown.enter="$emit('navigate', 'universe-detail', { id: universe.id })"
          @keydown.space.prevent="$emit('navigate', 'universe-detail', { id: universe.id })"
        >
          <!-- Gradient box with the universe initials -->
          <div
            class="flex h-14 w-14 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-emerald-500 via-teal-500 to-cyan-500 text-xl font-bold text-white"
          >
            {{ initials(universe) }}
          </div>
      ...
      >
        No universes yet. Add your first universe to get started.
      </p>
    </div>
    ...
    <AppModal
      v-if="modalOpen"
      :title="editingId ? 'Edit Universe' : 'New Universe'"
    ...
    >
      <form class="mt-6 space-y-4" @submit.prevent="save">
        ...
        >
          Name
          <AppTextField v-model="name" class="mt-1" placeholder="Universe name" />
        </label>
